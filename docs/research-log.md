...
## 25. Full-project validation pass (2026-08-17, Claude Code)

Independent re-verification of the whole project (3 parallel auditors + a deterministic structural audit) at the user's request ("make sure everything is correct, no hallucinations / wrong data").

**Structural/consistency audit (all 27):** clean — 0 high-severity issues. All 27 EU states present (no extras/dupes), every `code` matches its filename, every file with rates has a `tariffYear`, every `verified:true` has an official source, no future-dated `checkedAt`/`lastReviewed`, all URLs well-formed, translation preserved all numbers/URLs/dates. `arRequiredForForeignSellers=true` on all 27. Regulation timeline dates correct.

**Data-accuracy re-fetch (18 previously-verified countries, against cited official sources):** NO hallucinated rates found. Confirmed exactly against official tariff PDFs/pages: DE, ES, AT, BE (incl. wood 4.419), NL, IT, SE, FI, PT, HU, LV (subset), BG, CY, EE. Could not be machine-re-fetched (binary/image sources, not contradicted): DK, CZ (rates), HR, LU.
- **FR flipped to `verified:false`.** Its cited official CITEO 2026 rate guide is distributed as a `.7z` archive that cannot be opened; the CITEO rates page carries no numeric table; and independent 2026 summaries diverge both from the recorded rates and from each other (steel 27–199 €/t, glass 8–39 €/t across sources) — CITEO's contribution is per-unit + per-weight + eco-modulation, so a single €/kg per material is only indicative. Rates kept as indicative with a transparent caveat note; the `.7z` source marked `official:false`. Verified count 18 → 17.
- Minor (recorded for follow-up, not fixed): BG source lists a composite row (~0.174 €/kg) currently null; PT has a newer 2025 VPV table (file still on 2024); CZ €33 registration cost self-flagged unverified; FI paper 0.079 worth one human eyeball on the consumer column; PT rates use 4 decimals (convention is 3).
